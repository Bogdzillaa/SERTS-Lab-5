...
int32_t rx_msg_data2; // received data
...
osMessageQId mid_MsgQueue2;                     // message queue id 2
osMessageQDef (MsgQueue2, MSGQUEUE_OBJECTS, int32_t);    // message queue object 2
...
int Init_Thread (void) {
  ...
  mid_MsgQueue2 = osMessageCreate (osMessageQ(MsgQueue2), NULL);  // create msg queue 2
  if (!mid_MsgQueue) {
   ; // Message Queue object not created, handle failure
  }
...
void Thread_2 (void const *argument) {
  ...
  while (1) {
    ...
    osMessagePut (mid_MsgQueue2, rx_msg_data, osWaitForever); // Send Message
    LED_Off(1); delay();
   }
  }
}
